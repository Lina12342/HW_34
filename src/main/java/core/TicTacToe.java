...
public class TicTacToe {
	...
	static String checkWinnerAPI() {
		...
		for (int a = 0; a < 9; a++) {
			if (java.util.Arrays.asList(board).contains(String.valueOf(a + 1))) {
				break;
			} else if (a == 8) {
				return "draw";
			}
	...
	static String api(String in, boolean r) {
		if (in.length() < 5)
			return "Should be at least 5 turns";
		else if (in.length() > 9)
			return "Should be the most 9 turns";
		else if (!in.matches("^[1-9]+{5,9}$"))
			return "All turns must be a digit 1-9";
		else if (in.matches("^.*(.).*\\1.*$"))
			return "All turns must be unique";
		turn = "X";
		winner = null;
		board[0] = "1";
		board[1] = "2";
		...
		while (winner == null) {
			for (int i = 0; i < in.length(); i++) {
				...
				if (r) {
					System.out.print(turn + p + (i < in.length() - 1 ? " \u2192 " : "\n"));
				}
				;
